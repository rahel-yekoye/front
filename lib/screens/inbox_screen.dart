...
class _InboxScreenState extends State<InboxScreen>
  ...
  Future<void> _fetchConversations() async {
    final url = Uri.parse(
        'http://192.168.137.145:4000/conversations?user=${widget.currentUser}');
    try {
  ...
  Future<void> _fetchGroups() async {
    final url = Uri.parse('http://192.168.137.145:4000/groups');
    try {
      final response = await http.get(
        url,
        headers: {'Authorization': 'Bearer ${widget.jwtToken}'},
  ...
  Future<String> _fetchLastGroupMessage(String groupId) async {
    final url =
        Uri.parse('http://192.168.137.145:4000/groups/$groupId/last-message');
    try {
      final response = await http.get(
        url,
        headers: {'Authorization': 'Bearer ${widget.jwtToken}'},
